...
"use strict";angular.module("ordersApp",["ngRoute"]).config(["$routeProvider",function(a){a.when("/",{templateUrl:"partials/list.html",controller:"OrdersListController",title:"Orders List"}).when("/top-orders",{templateUrl:"partials/top.html",controller:"topOrdersController",title:"Top Ordered Items"}).otherwise({redirectTo:"/"})}]).controller("topOrdersController",["$scope","$http",function(a,b){var c=this;c.list=[],a.orders=c,c.get=function(){b.get("/api/top-orders").success(function(a){c.list=a.map(function(a){return a.orderId=a._id,delete a._id,a})})},c.get()}]).controller("OrdersListController",["$scope","$http",function(a,b){var c=this;c.list=[],c.orderToAdd={},c.searchInput="",a.orders=c,c.get=function(){b.get("/api/orders").success(function(a){c.list=a.map(function(a){return a.orderId=a._id,delete a._id,a})})},c.get(),c["delete"]=function(a){b["delete"]("/api/"+a).success(function(){c.list=c.list.filter(function(b){return b.orderId===a?!1:!0})})},c.search=function(){c.searchInput=c.searchInput.trim(),c.searchInput.length>0?b.get("/api/search/"+c.searchInput).success(function(a){c.list=a.map(function(a){return a.orderId=a._id,delete a._id,a})}):c.get()},c.add=function(){var a=c.orderToAdd;a.companyName&&a.customerAddress&&a.orderedItem&&a.companyName.length>0&&a.customerAddress.length>0&&a.orderedItem.length>0?b.post("/api/",c.orderToAdd).success(function(a){c.list.push({orderId:a._id,companyName:a.companyName,customerAddress:a.customerAddress,orderedItem:a.orderedItem}),c.orderToAdd={}}):$("#addOrderAlert").modal("show")}}]).run(["$rootScope",function(a){a.$on("$routeChangeSuccess",function(b,c,d){c.$$route&&(a.title=c.$$route.title)})}]),String.prototype.trim||(String.prototype.trim=function(){return this.replace(/^[\s\uFEFF\xA0]+|[\s\uFEFF\xA0]+$/g,"")});